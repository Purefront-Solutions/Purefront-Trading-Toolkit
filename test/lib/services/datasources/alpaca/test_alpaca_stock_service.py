...
def test_fetch_with_retries_returns_json_on_success():
    ...
    with patch("lib.services.datasources.alpaca.alpaca_stock_service.requests.get", return_value=mock_response):
...
def test_fetch_with_retries_does_not_retry_on_4xx():
    ...
    with patch("lib.services.datasources.alpaca.alpaca_stock_service.requests.get", return_value=mock_response) as mock_get:
...
def test_fetch_with_retries_retries_on_5xx_then_raises():
    ...
    mock_response.raise_for_status.side_effect = http_error
    with patch("lib.services.datasources.alpaca.alpaca_stock_service.requests.get", return_value=mock_response) as mock_get:
        with patch("lib.services.datasources.alpaca.alpaca_stock_service.time.sleep"):
            with pytest.raises(requests.exceptions.HTTPError):
                service._fetch_with_retries("https://example.com", {})
...
def test_fetch_with_retries_retries_on_connection_error_then_raises():
    service = AlpacaStockService(_make_config())
    with patch(
        "lib.services.datasources.alpaca.alpaca_stock_service.requests.get",
...
def test_test_connection_uses_correct_url():
    ...
    with patch("lib.services.datasources.alpaca.alpaca_stock_service.config", mock_sys_config):
        with patch("lib.services.datasources.alpaca.alpaca_stock_service.requests.get") as mock_get:
